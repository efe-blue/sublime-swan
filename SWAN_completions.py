...
class GetSettingsPrepare():
    def init(self):
        ...
        # 读取自己的设置文件
        settings = sublime.load_settings('SWAN_completions.sublime-settings')
...
class TagCompletions(sublime_plugin.EventListener):
    ...
    def on_query_completions(self, view, prefix, locations):
        ...
        # 仅针对swan有效
        if not view.match_selector(locations[0], "text.swan"):
            return []

        # 检测当前作用域是否在标签内
        is_inside_tag = view.match_selector(locations[0],
                "text.swan meta.tag - text.swan punctuation.definition.tag.begin")
    ...
    def get_attribute_completions(self, view, locations, prefix, ch):
        ...
        if ch == '"':
         ...
         # 属性名称的completion列表
         # 如果在字符串作用下则不输出
        elif not view.match_selector(locations[0], "text.swan string.quoted"):
            attr_data = {}
            attr_list = []
